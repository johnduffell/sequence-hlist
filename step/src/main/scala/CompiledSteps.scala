...
import WireCFN.{WireCode, WireEnvironment, WireFnGetAtt, WireFnSub, WireLambdaFunctionProperties, WireProps, WireResource, WireRoleProps, WireStateMachineProps}
import WireStateMachine.WireState
import com.amazonaws.services.lambda.runtime.Context
import play.api.libs.json._
import WireStateMachine._

import scala.util.Try

// this can be used to build the CFN and each lambda
case class CompiledSteps[FROM, FINAL](init: FROM => JsValue, steps: List[JsValue => Option[JsValue]], toResult: JsValue => Option[FINAL])
object CompiledSteps {
  ...
  def runLocal[FROM: Writes, FINAL](
    ...
    val fromString = compiledSteps.init(from)
    val finalString = compiledSteps.steps.foldLeft[Option[JsValue]](Some(fromString)){ case (str, nextStep) =>
      str.flatMap(nextStep)
    }
    finalString.flatMap(compiledSteps.toResult)
  ...
  def toCFN[FROM, FINAL](
    ...
    val nonTerminalstates = lambdas.sliding(2).toList.map({ case (stepData :: next :: Nil) =>
